...
- 클라라가 **`0917`** 입력 → **시작 리추얼**: ①pwr-clair/pwr-docs를 스크래치패드에 clone(최신) ②Notion 미러(PWR_MASTER, page 397b9d1f-a416-81f8-8e12-eeccfdefc21b)와 레포를 **전문 교차 대조 → 한쪽에만 있는 내용을 상호 병합**(⚠️**2026-07-30 클라라 확립: 헤더 날짜·시각 비교만으로 정본 판정 절대 금지** — 여러 창 마감이 몇 분 간격으로 얽혀 시각이 최신 내용을 보장하지 않음. 07-28 레포 21:55 마감 3분 뒤 Notion 21:58 마감으로 HK 방문고지 수리 기록이 유실될 뻔한 실사고) ③PWR_MASTER 기준 현재 상태·다음 액션(B1)·오늘 아젠다를 짧게 브리핑하고 대기.
...
- 클라라가 **"노션 싱크"** 입력 → **양쪽 전문 교차 대조·상호 병합(날짜 비교 금지, 07-30 개정)** 후 병합본을 Notion에 전체 교체로 쓰고 레포에도 동일 반영, 헤더 재조회 검증(챗 창에서 마스터 읽기 전 한 마디).
